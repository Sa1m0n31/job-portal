...
const AdminAddJobOffer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(jobOfferValidation()) {
            if(attachments.length + (oldAttachments ? oldAttachments.length : 0) <= 5) {
                setLoading(true);
                setError('');
                try {
                    if(updateMode) {
    ...
    return <div className="container container--addOffer">
        <LoggedAdminHeader />
        ...
        <main className="adminMain">
            ...
            <div className="adminMain__addOfferForm">
                ...
                <form className="addOffer" ref={addOfferForm}>
                    ...
                    <label className="admin--choseAgency">
                        Wybierz agencję
                        <select onChange={(e) => { setAgencyEmail(e.target.value); }}>
                            {agencies.map((item, index) => {
                                const agencyData = JSON.parse(item.data);
                                const agencyLabel = `${agencyData.name ? agencyData.name : 'Brak nazwy'} (${item.email})`;
...
export default AdminAddJobOffer;
